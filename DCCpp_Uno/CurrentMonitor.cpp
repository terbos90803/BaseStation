...
CurrentMonitor::CurrentMonitor(int pin_, float maxSamples_, const char *msg_)
  : pin(pin_), maxSamples(maxSamples_), msg(msg_)
{
  current = 0;
...
void CurrentMonitor::check() {
  ...
  if (current > maxSamples && digitalRead(SIGNAL_ENABLE_PIN_PROG) == HIGH) {              // current overload and Prog Signal is on (or could have checked Main Signal, since both are always on or off together)
